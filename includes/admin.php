...
function adduser()
{
	...
	if (mysqli_num_rows($query1)==0) {
		...
		$query = mysqli_query($con,$sql);
		if (!empty($query)) {
			echo "<br><b style='color:#008080;font-size:14px;font-family:Arial;'>User is Successfully Added</b>";
		}
	}
	else{
...
function addroom()
{
	...
	if (mysqli_num_rows($query1)==0) {
		...
		$query = mysqli_query($con,$sql);
		if (!empty($query)) {
			echo "<br><b style='color:#008080;font-size:14px;font-family:Arial;'>Room is Successfully Added</b>";
		}
		else{
...
function updateuser()
{
	...
	$name = $_GET['name'];
		...
		$query = mysqli_query($con,$sql);
		if (!empty($query)) {
			echo "<br><b style='color:#008080;font-size:14px;font-family:Arial;'>User is Successfully Updated</b>";
...
function updateroom()
{
	...
	$id = $_GET['id'];
		...
		$query = mysqli_query($con,$sql);
		if (!empty($query)) {
			echo "<br><b style='color:#008080;font-size:14px;font-family:Arial;'>Room is Successfully Updated</b>";
